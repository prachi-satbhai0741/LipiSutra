...
const getMockConfidence = (idx) => {
  const val = Math.abs(Math.sin(idx * 13));
  if (val > 0.97) return 0.20; // ~2% Red (Reconstruction Needed)
  if (val > 0.93) return 0.55; // ~3% Yellow (AI Prediction)
  return 0.95;                  // ~95% Plain (Historian Verified)
};
...
export default function PublicView() {
  ...
  const getPrecision = (transcript, aiOutput) => {
    const words = transcript.split(" ");
    const redCount = words.filter((w, idx) => {
      const ai = aiOutput?.find((a) => a.word === w) || aiOutput?.[idx];
      const confidence = ai ? ai.confidence : getMockConfidence(idx);
      return confidence <= 0.30;
    }).length;
    return Math.round(((words.length - redCount) / words.length) * 100);
  };
        ...
        <div className="flex flex-col items-center animate-[fadeIn_0.6s_ease-out]">
          ...
          >
            ...
            <p className="text-slate-400 font-light text-center text-sm md:text-base">
              Drag and drop a high-resolution image of your historical document, or{" "}
              <span className="text-gold-400 underline decoration-gold-500/30 underline-offset-4">browse files</span>.
            </p>
          ...
          <div className="lg:col-span-5 flex flex-col items-center">
            ...
            {!loading && !result && (
              <button
                ...
                className="mt-8 w-full bg-gradient-to-r from-gold-600 to-amber-700 text-museum-900 px-8 py-4 rounded-xl font-black text-lg tracking-widest uppercase hover:-translate-y-1 transition-transform shadow-[0_10px_30px_rgba(212,175,55,0.2)]"
              >
            ...
            <button
              ...
              className="mt-6 text-slate-500 hover:text-gold-400 transition-colors uppercase tracking-widest text-xs font-semibold underline underline-offset-8 decoration-museum-700 hover:decoration-gold-500/50"
            >
              ...
              <div className="w-full flex flex-col gap-12 animate-[fadeIn_0.5s_ease-out]">
                    ...
                    <div className="flex border-b border-white/10 mb-6 overflow-x-auto no-scrollbar gap-8 pb-1">
                      {[
                        ...
                        <button
                          ...
                          className={`pb-3 shrink-0 text-[10px] font-black tracking-[0.2em] uppercase transition-all duration-300 border-b-2 relative top-[2px] ${activeTab === tab.id
                        ...
                        <div className="animate-[fadeIn_0.4s_ease-out]">
                          ...
                          <div className="flex flex-col sm:flex-row gap-4 mb-10 bg-black/20 p-6 rounded-xl border border-white/5">
                            <select
                              ...
                              className="flex-1 bg-museum-900 border border-white/10 text-gold-400 px-6 py-4 rounded-xl focus:outline-none focus:border-gold-400 font-bold text-xs tracking-widest uppercase"
                            >
                          ...
                          {translation && (
                            <div className="bg-black/20 p-8 rounded-xl border-t-2 border-gold-500/50 shadow-inner">
                              ...
                              <button
                                ...
                                className="bg-[#C9A84C] text-museum-900 hover:bg-gold-400 px-10 py-4 rounded-xl text-[10px] font-black tracking-[0.2em] uppercase transition-all hover:-translate-y-1 shadow-2xl flex items-center justify-center gap-3 w-full sm:w-auto"
                              >
                ...
                <div className="w-full mt-4 animate-[fadeIn_0.6s_ease-out]">
                  <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 px-1">
                    <h3 className="text-xs font-bold tracking-[0.3em] text-slate-500 uppercase">Original Paleographic Transcript</h3>
                    <div className="flex items-center gap-4 bg-black/20 px-4 py-2 rounded-lg border border-white/5">
                      <span className="text-[10px] font-black tracking-widest text-slate-500 uppercase">Analysis Precision</span>
                      <div className="w-24 h-1.5 bg-white/5 rounded-full overflow-hidden">
                        <div
                          className="h-full bg-gold-500 shadow-[0_0_10px_rgba(212,175,55,0.3)] transition-all duration-1000"
                          style={{ width: `${getPrecision(result.transcript, result.ai_output)}%` }}
                        ></div>
                      </div>
                      <span className="text-xs font-bold text-gold-400">
                        {getPrecision(result.transcript, result.ai_output)}%
                      </span>
                    </div>
                  </div>

                  <div className="bg-museum-900/60 backdrop-blur-2xl p-10 md:p-16 rounded-3xl border border-white/5 shadow-2xl">
                    <div className="flex flex-wrap gap-y-3 gap-x-1 justify-center lg:justify-start" style={{ lineHeight: "2.6", wordSpacing: "4px" }}>
                      {result.transcript.split(" ").map((word, idx) => {
                        const ai = result.ai_output?.find((a) => a.word === word) || result.ai_output?.[idx];
                        const confidence = ai ? ai.confidence : getMockConfidence(idx);

                        // ~85% plain — Historian Verified (no box, just colored text)
                        if (confidence > 0.75) {
                          return (
                            <span
                              key={idx}
                              className="text-lg font-heading font-medium tracking-tight mx-0.5 self-center"
                              style={{ color: "#e8dcc8" }}
                              title="Historian Verified"
                            >
                              {word}
                            </span>
                          );
                        ...
                        // ~7% amber box — AI Prediction
                        if (confidence > 0.30) {
                          return (
                            <span
                              key={idx}
                              className="px-3 py-1 rounded text-sm font-medium mx-0.5 self-center"
                              style={{
                        ...
                        // ~8% red box — Reconstruction Needed
                        return (
                          <span
                            key={idx}
                            className="px-3 py-1 rounded text-sm font-medium mx-0.5 self-center"
                    ...
                    {/* Legend */}
                    <div className="flex flex-wrap items-center justify-center lg:justify-start gap-8 mt-12 pt-8 border-t border-white/5">
                      <div className="flex items-center gap-2">
                        <span className="w-3 h-3 rounded-full inline-block" style={{ backgroundColor: "#e8dcc8" }}></span>
                        <span className="text-[9px] font-black tracking-widest text-slate-500 uppercase">Historian Verified</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="w-3 h-3 rounded-full inline-block" style={{ backgroundColor: "#c8a96e" }}></span>
                        <span className="text-[9px] font-black tracking-widest text-slate-500 uppercase">AI Prediction</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="w-3 h-3 rounded-full inline-block" style={{ backgroundColor: "#cc6655" }}></span>
                        <span className="text-[9px] font-black tracking-widest text-slate-500 uppercase">Reconstruction Needed</span>
                      </div>
                    </div>
                  </div>
                </div>
